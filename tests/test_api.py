...
class APITestCase(unittest.TestCase):
    ...
    方法：
        get_api_headers: 返回所有请求都要发送的通用首部，其中包含认证密令和MIME类型相关的
            首部。
        test_no_auth: 确保Web服务会拒绝没有提供认证密令的请求，返回401错误代码。
        test_posts(): 把用户插入数据库，然后使用基于REST的API创建一篇博客文章，然后再读取
            这篇文章。

        所有请求主题中发送的数据都要使用json.dumps()进行编码，因为Flask测试客户端不会自动编
        码JSON格式数据。类似地，返回的响应主题也是JSON格式，处理之前必须使用json.load()方法
        解码。
    ...
    def test_users(self):
        ...
        r = Role.query.filter_by(name='User').first()
        self.assertIsNotNone(r)
        u1 = User(email='john@examole.com',
                  username='john',
                  password='cat',
                  confirmed=True,
                  role=r)
